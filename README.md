...
I wrote a program that builds a full 18-week NFL schedule from scratch. It figures out which team plays who, in which week, and at what time — then publishes the result as a website where you can look up any team's full schedule.
...
Once the weeks are set, each game gets a kickoff time. The program picks one primetime game each for Thursday, Sunday, and Monday nights, prioritizing good matchups between winning teams. It tries up to 500 different arrangements to find one where no team gets the same primetime slot two weeks in a row.

...
Combined NFL schedule generator:
  1. Reads games from opponents.py / games.txt
  2. Uses ILP (PuLP) to assign games to weeks with structural constraints
  3. Assigns kickoff times based on previous_records.txt win totals
  ...
  5. Writes final schedule to schedule_with_times.txt
"""
...
def load_records(filepath="previous_records.txt"):
    """Parse previous_records.txt → {team: wins}. Handles tie records like 9-7-1."""
    records = {}
...
def parse_previous_schedule(file_path):
    """Parse previous.txt → {week: [(home, away), ...]}."""
...
def main():
    ...
    output_file   = "schedule_with_times.txt"
